...
class comp():
    ...
    def getLCSC(self):
        return self.getField("LCSC Part #")
...
class netlist():
    ...
    def getLCSCComponents(self):
        """Return a subset of all components to be installed by JLCPCB (LCSC).
        Omit those that should not, by virtue of the fact that they don't contain an LCSC Part# field entry:
        """
    ...
    def getDigiKeySplitComponents(self):
        """Return a subset of all components to be purchased from Digi-Key for a Split build.
        Omit those that should not, by virtue of the fact that they contain an LCSC Part# field entry:
        """
        ...
        for c in self.components:
            include = True
            if (len(c.getLCSC()) > 0 or len(c.getDigiKey()) == 0) and c.getField("Build Type") != 'split':
                include = False
    ...
    def getDigiKeyComponents(self):
        """Return a subset of all components to be purchased from Digi-Key for a Full build.
        Omit those that should not, by virtue of the fact that they do not contain a Digi-Key Part field entry:
        """
